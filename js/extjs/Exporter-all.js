/*
 ...
 Edited by : Jimmy Rengga [j.rengga@gmail.com] 
 Adopted From: Steve Drucker
 October 26, 2013
...
Ext.override(Ext.grid.GridPanel, {
    ...
    getExcelXml: function(includeHidden, title) {
        ...
        return ''.concat('<?xml version="1.0"?>', '<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet" xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet" xmlns:html="http://www.w3.org/TR/REC-html40">', '<DocumentProperties xmlns="urn:schemas-microsoft-com:office:office"><Title>' + theTitle + '</Title></DocumentProperties>', '<OfficeDocumentSettings xmlns="urn:schemas-microsoft-com:office:office"><AllowPNG/></OfficeDocumentSettings>', '<ExcelWorkbook xmlns="urn:schemas-microsoft-com:office:excel">', '<WindowHeight>' + worksheet.height + '</WindowHeight>', '<WindowWidth>' + worksheet.width + '</WindowWidth>', '<ProtectStructure>False</ProtectStructure>', '<ProtectWindows>False</ProtectWindows>', '</ExcelWorkbook>',
                ...
                '<Style ss:ID="evendate" ss:Parent="even">', '<NumberFormat ss:Format="dd\\ mmm\\ yyyy" />', '</Style>',
                ...
                '<Style ss:ID="odd">', '<Interior ss:Color="#E6E6E6" ss:Pattern="Solid" />', '</Style>',
                '<Style ss:ID="odddate" ss:Parent="odd">', '<NumberFormat ss:Format="dd\\ mmm\\ yyyy" />', '</Style>',
    ...
    createWorksheet: function(includeHidden, theTitle) {
        ...
        var t = ''.concat('<Worksheet ss:Name="' + theTitle + '">',
                ...
                '<Table ss:ExpandedColumnCount="' + (visibleColumnCount + 2), '" ss:ExpandedRowCount="' + (this.store.getCount() + 2) + '" x:FullColumns="1" x:FullRows="1" ss:DefaultColumnWidth="65" ss:DefaultRowHeight="15">',
                ...
                if ((cm[j].dataIndex != '') && (includeHidden || !cm[j].hidden)) {
                    ...
                    if (cellType[k] !== "None") {
                        ...
                        if (cellType[k] == 'DateTime') {
                            ...
                            if (cm[j].dataIndex == 'STATUS_KONTRAK') {
                                if (v == '1') {
                                    t += 'AKTIF';
                                } else if (v == '2') {
                                    t += 'NON AKTIF / SELESAI';
                            ...
                            } else if (cm[j].dataIndex == 'STATUS_PROSES') {
                                if (v == '0') {
                                    t += 'Baru';
                                } else if (v == '1') {
                                    t += 'On Progress';
